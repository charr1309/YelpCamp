...
//require just the services needed from mapbox--for this app only geocoding is being used--all other services in docs at : https://github.com/mapbox/mapbox-sdk-js/blob/main/docs/services.md#forwardgeocode--lecture 541

const mbxGeocoding = require("@mapbox/mapbox-sdk/services/geocoding");

//grab that token from process.env.MAPBOX_TOKEN and save that to a variable, then pass that through when a new mapbox geocoding instance(mbxGeocoding) is instantiated--pass in the under the required key of accessToken where the token is what was just pulled from process.env

const mapBoxToken = process.env.MAPBOX_TOKEN;
const geocoder = mbxGeocoding({ accessToken: mapBoxToken }); //geocoder contains the two methods needed forward and reverse geocode
...
module.exports.renderNewForm = (req, res) => {
  //dont need async function here since its a form and wont be getting or posting until form is submitted
...
module.exports.createCampground = async (req, res, next) => {
  ...
  const geoData = await geocoder
    .forwardGeocode({
      //forwardGeocode takes on two properties, location and number of responses--limit defaults to 5 if nothing is specified
      query: req.body.campground.location,
      limit: 1,
    })
    .send();
  const campground = new Campground(req.body.campground);
  campground.geometry = geoData.body.features[0].geometry; //add geometry which is coming from geo coding api--lecture 542--it is saved on the campground instance
...
module.exports.showCampground = async (req, res) => {
  const campground = await Campground.findById(req.params.id)
    .populate({
      path: "reviews",
      populate: {
        //set an object on this nested populate--populate all the reviews on the show page for the campground we are finding, populate the reviews, then populate their author, and then seperately below in the second populate, populate the author of this campground
        path: "author",
      },
    })
...
module.exports.renderEditForm = async (req, res) => {
  //isAuthor validates if user can edit a campground
  const { id } = req.params;
