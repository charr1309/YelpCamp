...
const MongoSanitize = require('express-mongo-sanitize');//added npm package to prohibit special characters--lecture 561--sql injection-when a user injects some code into the query string to for example hack into a database
...
app.use(MongoSanitize({//tell mongo to use mongoSanitize to prohibit special characters in the query string
    replaceWith: '_'//tells mongo to replace any special character with an underscore
}))

const sessionConfig = {//configure session which will be an object
    name: 'session',//set a name so the default connect.sid is not used
    ...
    cookie: {
        ...
        //secure: true,//means this cookie should only work or be configured under HTTPS(secured server)
...
app.use(flash());
app.use(helmet());//lecture 566--prevents click jacking attacks cross side scripting(XSS) attacks
