...
router
  ...
  ); //in the form data the field created was image, so that is the name used in upload.array

// .post(upload.array('image'), (req, res) =>{//this is uploading from multer with upload.single or upload.array for example, and told multer to store things in a different destination above,--lecture 527--upload.single is multer middelware that adds a body object and a file or files object to the request object
...
router
  ...
  .put(
    isLoggedIn,
    isAuthor,
    upload.array('image'),//added lecture 534 to allow multiple images in an array
    validateCampground,
